...
def save_misclassified_frames_to_pdf(model, x_test, y_test, path):
    predictions = model.predict(x_test)
    y_pred = np.argmax(predictions, axis=1)
    misclassified_indices = np.where(y_pred != y_test)[0]

    with PdfPages(path + '/misclassified_frames.pdf') as pdf:
        for idx in misclassified_indices:
            frame = x_test[idx].reshape(16, 10)
            plt.figure()
...
def main(train_path, test_path, outpath, epochs, batch_size, num_classes, n_splits=5):
    ...
    save_misclassified_frames_to_pdf(best_model, x_test, y_test, outpath)
