...
# Custom Keras layer for keeping top 32 elements
class KeepTop32(tf.keras.layers.Layer):
    def __init__(self, **kwargs):
        super(KeepTop32, self).__init__(**kwargs)

    def call(self, inputs):
        values, _ = tf.nn.top_k(tf.reshape(inputs, [-1, 160]), k=32)
        min_value = values[:, -1:]
        mask = tf.cast(tf.greater_equal(inputs, min_value), tf.float32)
        return inputs * mask
