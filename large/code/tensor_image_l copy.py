...
# 加载训练好的模型
model = tf.saved_model.load(r'D:\repository\deeplearning\large\Data\tensorflow\ncz\1')
logging.info("模型加载成功")
...
def update_heatmap(matrix, top_n=300):
        ...
        while running:
            ...
            try:
                ...
                if frameData_b is not None:
                    print(f"New matrix data collected: shape {frameData_b.shape}")  # 调试输出
    ...
    def run(self):
        ...
        while running:
            ...
            if current_time - last_inference_time >= self.inference_interval and matrix_buffer:
                matrix = matrix_buffer[-1]  # 使用最新的矩阵数据
                try:
                    ...
                    print(f"新预测结果: {latest_prediction}")  # 调试输出
                    
                    while not inference_results.empty():
                        inference_results.get()  # 清空旧结果
...
def main():
    global alld, ser, latest_prediction, running, posture_labels
    ...
    # 新增：连接数据采集线程的信号到GUI的receive_new_data方法
